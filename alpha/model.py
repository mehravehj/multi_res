...
class sConv_multi(nn.Module):
    def __init__(self, in_, out_, kernel_size, max_scales=4, classifier=0):
        ...
        if classifier: # 1 for last layer
            self.channel_per_res = [out_ for i in range(max_scales)] # number of filters for last layer is the same for all resolusions
        else:
            self.channel_per_res = [out_*(2)**i for i in range(max_scales)] # number of filters per resolution
        self.out_ = self.channel_per_res[-1] # number of channels for the output of the layer
        self.filters = torch.empty(self.out_, in_, kernel_size, kernel_size) # create filters
        k = sqrt(1 / (in_ * kernel_size ** 2)) # boundary for data distribution
        nn.init.uniform_(self.filters, a=-k, b=k) #initialize weight from uniform distribution
        self.filters = nn.Parameter(self.filters)
    ...
    def forward(self, x):
        ...
        for r in range(self.max_scales):
            y = F.conv2d(lx[r], self.filters[:self.channel_per_res[r], :, :, :], padding=1) # apply convolution using
            ...
            pad = (0, 0, 0, 0, 0, self.out_ - self.channel_per_res[r]) # zero padding along channel dimension
            ly.append(F.pad(y_up, pad))
        y = (torch.stack(ly, 0))
        ys = y.shape
...
class multires_model(nn.Module):
    def __init__(self, ncat=10, channels=16, leng=10, max_scales=4):
        ...
        listc = [sConv_multi(3, channels, 3, max_scales)]
        listc += [sConv_multi(channels * 2**(max_scales-1), channels, 3, max_scales) for i in range(leng - 2)]
        listc += [sConv_multi(channels * 2**(max_scales-1), ncat, 3, max_scales, 1)]
